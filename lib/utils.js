...
exports.getPatternedFiles=(dir, patterns,partial)=>{
    var pats=[];
    patterns.forEach(pat=>{
        if(pat.includes("*"))
            pats.push(new minimatch.Minimatch(pat,{matchBase:true,nocase:true}));
    });
    if(pats.length){
        return fs.readdirSync(dir)
                .filter(entry=>pats
                    .some(rule=>rule.match('/' + entry) || rule.match(entry) ||
